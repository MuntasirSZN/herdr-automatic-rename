...
- **The names reach your window title.** herdr `>= 0.8.2` writes the outer terminal's title itself, `{hostname}: {workspace}` by default, so a workspace's `[N]` prefix already shows there. Add `{tab}` to carry the tab name too, agent task included: `window_title = "{hostname}: {workspace} · {tab}"` under `[ui]` in `~/.config/herdr/config.toml`. herdr's own `{terminal_title}` token shows the focused pane's raw title instead; `{tab}` is the label this plugin wrote, prefix and icon included.
