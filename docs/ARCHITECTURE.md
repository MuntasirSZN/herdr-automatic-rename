...
A `cd` emits no event the plugin subscribes to, so an event-driven pass learns of it whenever the next event happens to arrive, which is what left the workspace sitting on its creation directory while the tab beside it followed every prompt ([#20](https://github.com/qu8n/herdr-automatic-rename/issues/20)). The shell hook is the thing that does fire on a cd, so `ar_fast_workspace` runs the same rules from precmd: the directory is the shell's own `$PWD`, the base is `ar_project_base` of it, and `ar_ws_track_eligible` decides whether it may be applied. `workspace.updated` (herdr 0.8) is the event that would cover a pane with no hook installed, and is not subscribed here because a hook naming an event an older herdr does not know is untested against 0.7.x.

Two things bound what the prompt pays. The workspace we own is read out of the state file, so a prompt that derives the base already recorded asks herdr nothing, and only a cd that leaves the project reaches `workspace list`. And a workspace nobody has adopted, or one somebody named by hand, is left to the reconcile, because adopting one means reading its label and that is the round-trip the guard exists to refuse. The workspace id comes off the tab id, which carries it in front of a colon, so nothing is fetched to find it.

Which knobs govern this is the workspace's own question, not the tab's: the half runs where `ar_index_pass workspaces` says the reconcile would number or strip, whatever `NAME_TABS` says. preexec skips it, since a cd has landed by the time the prompt is drawn and preexec's `$PWD` is the one the last precmd already saw.
...
Each refusal returns the empty string, and naming carries on to the program, `PROGRAM_ALIASES` and the `WRAPPER_PROGRAMS` unwrapping included. A title that survives replaces the program name outright, alias and all: `AGENT_TITLES` is the request for the task, and an alias shortening `claude` to `cl` is not a request to hide the work. `TITLE_STYLE=name_and_task` asks for it back, and there the alias does apply, since asking for the name is asking for the name you chose for it; the name and its colon are charged to the same budget, and both are dropped when what is left would not seat `MIN_TASK_LEN` characters of task. The program still supplies the icon, and the label gets its own budget, `MAX_TITLE_LEN` (28) rather than `MAX_NAME_LEN` (20), cut at a word boundary when that leaves at least half of it. A title is a sentence, and `Investigate` says more than `I` does.
